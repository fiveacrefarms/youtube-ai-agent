...
def concatenate_clips(output_file="concatenated.mp4"):
    ...
    """
    # Ensure the video_clips folder exists
    if not os.path.exists(VIDEO_CLIPS_DIR):
        print(f"[ERROR] The folder '{VIDEO_CLIPS_DIR}' does not exist!")
        print("[INFO] Creating the folder. Please add video clips to process.")
        os.makedirs(VIDEO_CLIPS_DIR)  # Create the folder
        return  # Exit the function since we cannot proceed without clips

    # Check if the folder contains any .mp4 files
    video_files = sorted([clip for clip in os.listdir(VIDEO_CLIPS_DIR) if clip.endswith(".mp4")])
    if not video_files:
        print(f"[ERROR] No video clips found in the folder '{VIDEO_CLIPS_DIR}'.")
        print("[INFO] Please add at least one .mp4 file to the folder.")
        return  # Exit the function since we cannot proceed without clips
...
def assemble_video():
    ...
    if os.path.exists(concatenated_video):
        ...
        print("[ERROR] Concatenated video not found. Assembly process stopped.")
